...
RENAME COLUMN `п»їdate_new` TO date_new;

ALTER TABLE customer_info
RENAME COLUMN `п»їId_client` TO Id_client;

SELECT *
FROM customer_info
LIMIT 1000;

ALTER TABLE transactions_excel1
RENAME COLUMN `п»їdata_new` TO data_new;
...
USE test77;
# запрос для 2a-d
SELECT   DATE_FORMAT(t.data_new, '%Y-%m') AS month_number,
    ...
    ROUND(
        ...
        (SELECT COUNT(DISTINCT Id_check)
            FROM transactions_excel1
            WHERE data_new >= '2015-06-01'
              AND data_new < '2016-06-01'), 2) AS operations_share_percent,
    ROUND(SUM(t.Sum_payment) * 100.0 /(SELECT SUM(Sum_payment)
            FROM transactions_excel1
            WHERE data_new >= '2015-06-01'
              AND data_new < '2016-06-01'),2) AS sum_share_percent
FROM transactions_excel1 AS t
WHERE t.data_new >= '2015-06-01'
  AND t.data_new < '2016-06-01'
...
WITH gender_month AS (
    ...
    WHERE t.data_new >= '2015-06-01'
      AND t.data_new < '2016-06-01'
    GROUP BY
        DATE_FORMAT(t.data_new, '%Y-%m'),
        COALESCE(NULLIF(c.Gender, ''), 'NA')
),
month_total AS (    SELECT        DATE_FORMAT(data_new, '%Y-%m') AS month_number,
...
SELECT   gm.month_number,    gm.gender_group,    gm.clients_count,    ROUND(gm.clients_count * 100.0 / mt.total_clients, 2) AS clients_share_percent,
    ROUND(gm.gender_sum, 2) AS gender_sum,
    ROUND(gm.gender_sum * 100.0 / mt.total_sum, 2) AS spending_share_percent
FROM gender_month AS gm
JOIN month_total AS mt
